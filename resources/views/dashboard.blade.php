...
@section('contentss')
    ...
    <div class="container">
            ...
            <div class="row justify-between g-2 gy-3">
                <div class="col-12 col-md-9">
                    <div class="profile-pix flex items-center">
                        ...
                        <img src="{{ asset('uploads/images/' . $getUser->image) }}" class="profile-img" alt="">
                ...
                $raffle = DB::table('raffle_order')->where('raffle_order.user_id', session()->get('user_id'))->join('raffle','raffle_order.raffle_id','raffle.id')->where('raffle.ending_date' ,'>', now()->format('Y-m-d H:i:s') )
                ->leftJoin('organisation', 'raffle.organisation_id', 'organisation.id')
                ->select('raffle.*', 'organisation.organisation_name', 'organisation.cover_image', 'organisation.handle', 'organisation.website',  DB::raw('SUM(raffle_order.amount) as total_amount'))->groupBy('raffle.id')->limit(3)->get();
                $raffleEx = DB::table('raffle_order')->where('raffle_order.user_id', session()->get('user_id'))->join('raffle','raffle_order.raffle_id','raffle.id')->where('raffle.ending_date' ,'<=', now()->format('Y-m-d H:i:s') )
                ->leftJoin('organisation', 'raffle.organisation_id', 'organisation.id')
                ->select('raffle.*', 'organisation.organisation_name', 'organisation.cover_image', 'organisation.handle', 'organisation.website',  DB::raw('SUM(raffle_order.amount) as total_amount'))->groupBy('raffle.id')->limit(3)->get();
        ...
        @endphp
            ...
            <div class="rafle">
                <h5 class="head">Ongoing Raffles</h5>
            </div>
                ...
                <div class="">
                    <div class="raffle-group">
                        @foreach ($raffle as $item)
                            <div class=" raffle-cardz" onclick="location.href='{{ url('raffle_detail/' . $item->state_raffle_hosted) }}'">
                                <div class="img">
                                ...
                                <div class="text" style="position: relative">
                                    ...
                                    <div class="bottom-text flex justify-between align-items-center">
                                        ...
                                        <h6>Amount <span class="block text-right" style="text-align: right">${{$item->total_amount}}</span>
                                        </h6>
                                    </div>
                                </div>
            ...
            <div class="rafle mt-3">
                <h5 class="head">Ended Raffles</h5>
            </div>
            <div class="ongoing-card-container">
