...
class AdminRouteController extends Controller
{
    ...
    {
        if (session()->has('user_id') && session()->get('user_type') == 'admin') {
            $data = DB::table("raffle")->where('raffle.id', $id)->leftJoin('users', 'raffle.user_id', 'users.id')->select('raffle.*', 'users.first_name', 'users.last_name', 'users.email', 'users.image', 'users.about', 'users.username')->first();
            if ($data) {
